...
... | e₃ , b , c , step₁ , step₂ , p    = 
    `app e₃ _ , _ , _ , 
      ...
      ↦*-trans (compatible se-app step₂) (compatible se-app₁ (⇓→↦* d₁)) , 
        Eq.trans (cong (λ a → a ∙ _) p) (assoc _ _ _)
...
... | ↦*-refl            | ↦*-step step steps  =                
    `app (`fun _) _ , _ , _ , 
      ...
      ↦*-trans (compatible se-app₁ steps) (↦*-step (se-app₂ v-val) (⇓→↦* d₂)) , 
        Eq.trans (cong (λ a → a ∙ _) (identityˡ _)) (Eq.trans (assoc _ _ _) (cong (λ a → _ ∙ (_ ∙ a)) (sym ((identityˡ _)))))
... | ↦*-step step steps | _                   = 
    `app _ _ , _ , _ , 
      se-app step , 
      ↦*-trans (compatible se-app steps) (↦*-trans (compatible se-app₁ (⇓→↦* d₁)) (↦*-step (se-app₂ v-val) (⇓→↦* d₂))) , 
        Eq.trans (assoc _ _ _) (Eq.trans (assoc _ _ _) (cong (λ a → _ ∙ (_ ∙ (_ ∙ a))) (sym (identityˡ _)))) 
...
... | ↦*-step step steps = 
    `case _ _ _ , _ , _ , 
      ...
      ↦*-trans (compatible se-case steps) (↦*-step se-case-z (⇓→↦* d₁)) , 
        Eq.trans (assoc _ _ _) (cong (λ d → _ ∙ (_ ∙ d)) (sym (identityˡ _))) 
