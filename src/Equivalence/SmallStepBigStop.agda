...
module Equivalence.SmallStepBigStop {ℓ : Level} (monoid : MonoidWithLeftZero ℓ) where
...
↦*→⇓ (↦*-step e↦e''↝a e''↦*e'↝b) = ↦→⇓ e↦e''↝a (↦*→⇓ e''↦*e'↝b)
  where
    ...
    ↦→⇓ (se-case {a = a} e↦e') (ste-case-z {a = b} {b = c} e'⇓z e₁⇓e'') rewrite sym (assoc a b c) 
      = ste-case-z (↦→⇓ e↦e' e'⇓z) e₁⇓e''
    ↦→⇓ (se-case {a = a} e↦e') (ste-case-s {a = b} {b = c} e'⇓s v-val e₂⇓e'') rewrite sym (assoc a b c)
    ...
    ↦→⇓ (se-app e↦e') (ste-app-seq₁ e'⇓e'') 
      = ste-app-seq₁ (↦→⇓ e↦e' e'⇓e'')
    ↦→⇓ {a = a} (se-app e↦e') (ste-app-seq₂ {a = b} {b = c} e'⇓e'' e'⇓e''') rewrite sym (assoc a b c) 
      = ste-app-seq₂ (↦→⇓ e↦e' e'⇓e'') e'⇓e'''
    ↦→⇓ {a = a} (se-app e↦e') (ste-app {a = b} {b = c} {c = d} e'⇓e'' e'⇓e''' v-val e'⇓e'''') rewrite Eq.trans (sym (assoc a (b ∙ c) d)) (cong (λ a → a ∙ d) (sym (assoc a b c)))
      = ste-app (↦→⇓ e↦e' e'⇓e'') e'⇓e''' v-val e'⇓e''''
    ↦→⇓ (se-app e↦e') ste-stop 
      = ste-app-seq₁ (↦→⇓ e↦e' ste-stop)
    ↦→⇓ {a = a} (se-app₁ e↦e') (ste-app-seq₁ ste-fun) rewrite sym (identityˡ (a ∙ 1#)) 
      = ste-app-seq₂ ste-fun (↦→⇓ e↦e' ste-stop)
    ↦→⇓ {a = a} (se-app₁ e↦e') (ste-app-seq₁ ste-stop) rewrite sym (identityˡ (a ∙ 1#)) 
      = ste-app-seq₂ ste-fun (↦→⇓ e↦e' ste-stop)
    ↦→⇓ {a = a} (se-app₁ e↦e') (ste-app-seq₂ {b = b} ste-fun e'⇓e'') rewrite Eq.trans (Eq.cong (λ b → a ∙ b) (identityˡ b)) (sym (identityˡ (a ∙ b)))
      = ste-app-seq₂ ste-fun (↦→⇓ e↦e' e'⇓e'')
    ↦→⇓ {a = a} (se-app₁ e↦e') (ste-app-seq₂ {b = b} ste-stop e'⇓e'') rewrite Eq.trans (Eq.cong (λ b → a ∙ b) (identityˡ b)) (sym (identityˡ (a ∙ b))) 
      = ste-app-seq₂ ste-fun (↦→⇓ e↦e' e'⇓e'')
    ↦→⇓ {a = a} (se-app₁ e↦e') (ste-app {a = b} {b = c} {c = d} ste-fun e'⇓e''' v-val e'⇓e'''') 
      rewrite Eq.trans (cong (λ c → a ∙ (c ∙ d)) (identityˡ c)) (Eq.trans (sym (assoc a c d)) (Eq.trans (sym (identityˡ (a ∙ c ∙ d))) (sym (assoc 1# (a ∙ c) d))))
      = ste-app ste-fun (↦→⇓ e↦e' e'⇓e''') v-val e'⇓e''''
    ↦→⇓ {a = a} (se-app₁ e↦e') (ste-app {a = b} {b = c} {c = d} ste-stop e'⇓e''' v-val e'⇓e'''') 
      rewrite Eq.trans (cong (λ c → a ∙ (c ∙ d)) (identityˡ c)) (Eq.trans (sym (assoc a c d)) (Eq.trans (sym (identityˡ (a ∙ c ∙ d))) (sym (assoc 1# (a ∙ c) d))))
      = ste-app ste-fun (↦→⇓ e↦e' e'⇓e''') v-val e'⇓e''''
    ↦→⇓ {a = a} (se-app₁ e↦e') ste-stop rewrite sym (identityˡ (a ∙ 1#))
      = ste-app-seq₂ ste-fun (↦→⇓ e↦e' ste-stop)
    ↦→⇓ {b = b} (se-app₂ v-val) e'⇓e'' rewrite (Eq.cong (λ a → a ∙ b) (sym (identityˡ 1#)))
